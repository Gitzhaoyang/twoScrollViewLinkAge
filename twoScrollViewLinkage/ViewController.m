...
#define KFIRST_SCROLLVIEW_HEIGHT 40
...
#define KFIRST_SCROLLVIEW_ITEM_WIDTH 100
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addFirstScrollViewOnView];
    [self addSecondScrollViewOnView];
}
...
- (void) addFirstScrollViewOnView {
    ...
    for (int i = 0; i < self.itemsTitlesArray.count; i ++) {
        ...
        itemButton.backgroundColor = [UIColor colorWithRed:0.94 green:0.94 blue:0.94 alpha:1.0];//背景颜色
        [itemButton setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];//标题颜色
    ...
    //**3.添加选中按钮的背景
    UIImageView *selectedImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, KFIRST_SCROLLVIEW_ITEM_WIDTH, KFIRST_SCROLLVIEW_HEIGHT)];
    selectedImageView.image = [UIImage imageNamed:@"selected_btn"];
    self.selectedButtonImageView = selectedImageView;
    [firstScrollView addSubview:selectedImageView];//添加到第一个scrollView
...
- (void) addSecondScrollViewOnView {
    ...
    for (int i = 0; i < self.itemsTitlesArray.count; i ++) {
        ...
        bottomView.backgroundColor = [self.colorArray objectAtIndex:i];
